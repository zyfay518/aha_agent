...
输入访问码、刚创建的 `item_id`，并提供以下一种图片来源：

- ChatGPT 标准文件对象 `file`，包含必需的 `download_url`、`file_id` 和可选的 `mime_type`、`file_name`；工具通过 `_meta["openai/fileParams"] = ["file"]` 声明文件参数。
- 宿主临时 HTTPS 图片地址 `file_url`。
- Codex 本地编辑结果的 `image_base64` 与 `mime_type`。

服务端校验图片格式和大小，并统一转为白底 JPEG 目录图后覆盖该单品的展示图片。上身照场景只能提交经过用户确认的单品预览，禁止把原始人物照片直接绑定到衣橱记录。

### 上身照录入决策

1. Agent 判断图片是否包含人物和多件可穿戴物。
2. 用户已明确唯一目标时直接进入该目标的提取；否则先询问要添加哪一件，未确认前不调用新增工具。
