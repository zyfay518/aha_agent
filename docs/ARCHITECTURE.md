...
- 生成存储路径。
- 原图和缩略图管理。
- 接收 ChatGPT Apps SDK 标准文件参数、临时 HTTPS 地址或 Codex Base64 编辑结果。
- 上身照的识别和服装重建由宿主 Agent 完成；后端只规范化并保存用户确认后的白底单品图，不保存原始人物照作为衣橱展示图，也不调用图像模型 API。
- 签名访问地址。
- 删除任务。
...
- 穿搭板内各单品先对目录图边缘执行近白背景连通域去除，再由 Sharp 并行缩放为透明 PNG 图层；最终在浅莫兰迪纯色底图上按“上装在上、下装居中、鞋履在下、包袋两侧”的穿搭关系排版，并根据品类设置相对比例。
- 登录后的备份衣橱页面使用 `createSignedUrls` 批量申请图片签名地址，由每件一次 Storage 请求降为整页一次。
- 多件平铺图由宿主 Agent 完成目标检测、逐件分离和白底预览；MCP 不调用后端视觉模型。`add_wardrobe_items_batch` 接收 2–8 份已确认的预览，图片规格化后通过一个 Postgres 事务同时写入 `wardrobe_items` 与 `agent_item_images`，避免批量中途产生半成品。
- 为 `agent_access_tokens.user_id` 和 `agent_item_images.user_id` 补充外键覆盖索引；旧 RPC、旧索引与返回契约均保留。
- 输入图片增加 4000 万像素解码上限和顺序读取，避免超大压缩图占用过多运行内存；现有 8 MB 文件大小限制不变，穿搭板固定输出 1200×1200 JPEG。
